...
const searchCatName = (req, res) => {
  ...
  return Cat.findByName(req.query.name, (err, cat) => {
    // errs, handle them
    if (err) {
      return res.status(500).json({ err }); // if error, return it
    }
    ...
    return res.json({ name: cat.name, beds: cat.bedsOwned });
  });
};
...
const searchDogName = (req, res) => {
  ...
  return Dog.findByName(req.query.name, (err, dog) => {
    // errs, handle them
    if (err) {
      return res.status(500).json({ err }); // if error, return it
    }
    ...
    // (does not necessarily have to be an error since technically it worked correctly)
    if (!dog) {
      return res.json({ error: `No dog named ${req.query.name} found` });
    }

    // if a match, assign dog to lastDog and increase age
    lastDogAdded = dog;
    updateLastDog(req, req);
